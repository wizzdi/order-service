...
public class OrderService
		implements
			com.flexicore.order.interfaces.IOrderService {
	...
	public PaginationResponse<Order> getAllOrders(
			OrderFiltering orderFiltering, SecurityContext securityContext) {
		List<Order> list = listAllOrders(orderFiltering, securityContext);
		long count = orderRepository.countAllOrders(orderFiltering,
				securityContext);
	...
	@Override
	public void validate(OrderFiltering orderFiltering,
			SecurityContext securityContext) {
		Set<String> consumingOrganizationIds = orderFiltering
				.getConsumingOrganizationIds();
		Map<String, Organization> organizationMap = !consumingOrganizationIds
				.isEmpty()
				? listByIds(Organization.class, consumingOrganizationIds,
						securityContext).parallelStream().collect(
		...
		if (!consumingOrganizationIds.isEmpty()) {
			throw new BadRequestException("No Organizations with ids "
					+ consumingOrganizationIds);
		}
		orderFiltering.setConsumingOrganizations(new ArrayList<>(
				organizationMap.values()));

		Set<String> supplierIds = orderFiltering.getSupplierIds();
		Map<String, Supplier> supplierMap = !supplierIds.isEmpty()
		...
		if (!supplierIds.isEmpty()) {
			throw new BadRequestException("No Suppliers with ids "
					+ supplierIds);
		}
	...
	@Override
	public void validate(CreateOrder createOrder,
			SecurityContext securityContext) {
	...
	@Override
	public void validate(UpdateOrder updateOrder,
			SecurityContext securityContext) {
		Order Order = orderRepository.getByIdOrNull(updateOrder.getId(),
				Order.class, null, securityContext);
		if (Order == null) {
			throw new BadRequestException("no Order with id "
					+ updateOrder.getId());
		}
	...
	private void validateUpsertOrder(CreateOrder createOrder,
			SecurityContext securityContext) {
		String consumingOrganizationId = createOrder
				.getConsumingOrganizationId();
		Organization consumingOrganization = consumingOrganizationId != null
				? getByIdOrNull(consumingOrganizationId, Organization.class,
						null, securityContext) : null;
		if (consumingOrganization == null && consumingOrganizationId != null) {
			throw new BadRequestException("No Organization with id "
		...
		Supplier supplier = supplierId != null ? getByIdOrNull(supplierId,
				Supplier.class, null, securityContext) : null;
	...
	@Override
	public Order createOrderNoMerge(CreateOrder createOrder,
			SecurityContext securityContext) {
		Order order = new Order(createOrder.getName(),
				securityContext);
	...
	public boolean updateOrderNoMerge(Order order, CreateOrder createOrder) {
		...
		if (createOrder.getName() != null
				&& !createOrder.getName().equals(order.getName())) {
			order.setName(createOrder.getName());
			update = true;
		}
		if (createOrder.getDescription() != null
				&& !createOrder.getDescription().equals(order.getDescription())) {
			order.setDescription(createOrder.getDescription());
			update = true;
		}
		if (createOrder.getExternalId() != null
				&& !createOrder.getExternalId().equals(order.getExternalId())) {
			order.setExternalId(createOrder.getExternalId());
			update = true;
		}
		if (createOrder.getConsumingOrganization() != null
				&& (order.getConsumingOrganization() == null || !createOrder
						.getConsumingOrganization().getId()
						.equals(order.getConsumingOrganization().getId()))) {
			order.setConsumingOrganization(createOrder
					.getConsumingOrganization());
			update = true;
		}
		if (createOrder.getSupplier() != null
				&& (order.getSupplier() == null || !createOrder.getSupplier()
						.getId().equals(order.getSupplier().getId()))) {
			order.setSupplier(createOrder.getSupplier());
			update = true;
		}

		if (createOrder.getOrderDate() != null
				&& (order.getOrderDate() == null || !createOrder.getOrderDate()
						.equals(order.getOrderDate()))) {
			order.setOrderDate(createOrder.getOrderDate());
			update = true;
		}

		if (createOrder.getOrderSentDate() != null
				&& (order.getOrderSentDate() == null || !createOrder
						.getOrderSentDate().equals(order.getOrderSentDate()))) {
			order.setOrderSentDate(createOrder.getOrderSentDate());
			update = true;
		}

		if (createOrder.getOrdinal() != null
				&& createOrder.getOrdinal() != order.getOrdinal()) {
	...
	public <T extends Baseclass> T getByIdOrNull(String id, Class<T> c,
		...
		return orderRepository.getByIdOrNull(id, c, batchString,
				securityContext);
	}
}
